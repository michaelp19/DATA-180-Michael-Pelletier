...
#library(tidyverse)
library(tm)
news<-read.csv("/Users/michaelpelletier/Desktop/Data 180-Michael Pelletier version2 /DATA-180-Michael-Pelletier/hw7/news.csv",header=T)
```

Read also the positive and negative word dictionaries, both found on Github. This will later come in handy:

```{r}
posWords <- scan("/Users/michaelpelletier/Desktop/Data 180-Michael Pelletier version2 /DATA-180-Michael-Pelletier/hw7/positive-words.txt", character(0), sep = "\n")  # 2006 items
negWords <- scan("/Users/michaelpelletier/Desktop/Data 180-Michael Pelletier version2 /DATA-180-Michael-Pelletier/hw7/negative-words.txt", character(0), sep = "\n")  # 4783 items
head(posWords,15)
head(negWords,15)
```
...
term_frequencies <- row_sums(as.matrix(dtm))

word_pairs <- colnames(dtm)[term_frequencies > 0]

word_pairs <- word_pairs[!grepl("[[:punct:]]", word_pairs)]
word_pairs <- word_pairs[!grepl("[[:digit:]]", word_pairs)]

top20_word_pairs <- head(sort(word_pairs, decreasing = TRUE), 20)

print(top20_word_pairs)
```
