...
```{r}
clust <- hclust(dist(Mammals_Milk_Std, method = "euclidean"), method = "complete")

plot(clust, main = "Mammals Clustered by Std. Milk Composition \n Euclidean Metric, Complete Linkage",
     sub = "", xlab = "Mammals", cex.lab = 1.25, hang = -1, frame.plot = TRUE, lwd = 3)
...
sorted_data_km_3 <- Mammals_Milk[order(Mammals_Milk$km_3), ]
sorted_data_km_4 <- Mammals_Milk[order(Mammals_Milk$km_4), ]

print("Sorted Table for K = 3:")
print(sorted_data_km_3)

print("Sorted Table for K = 4:")
print(sorted_data_km_4)
...
for (k in 1:8) {
  kmeans_result <- kmeans(scale(Mammals_Milk), centers = k, nstart = 50)
  wss_values <- c(wss_values, kmeans_result$tot.withinss)
}

elbow_data <- data.frame(K = 1:8, WGSS = wss_values)

elbow_plot <- ggplot(elbow_data, aes(x = K, y = WGSS)) +
  geom_line() +
  geom_point() +
  labs(title = "Elbow Plot for Optimal K") +
  xlab("Number of Clusters (K)") +
  ylab("Within-Group Sum of Squares (WGSS)")

print(elbow_plot)
#  How many clusters does the elbow plot suggest? How does this compare to your impression from the dendrogram in Question 2?
