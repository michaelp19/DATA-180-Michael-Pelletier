...
count_records_broken_more_than_a_year <- records %>%
  filter(record_duration > 365) %>%
  nrow()  


print(count_records_broken_more_than_a_year)
...
print(longest_duration_record[, c("player", "track", "date", "record_duration")])
...
fastest_track <- track_means %>%
  filter(mean_time == min(mean_time))



cat("The track with the fastest average record time is:", fastest_track$track)
...
#Do you notice a trend across years? Are the record times getting faster, slower, or no clear pattern in time?
#I don't see a trend across the years. There is no clear pattern in the time and the year. 2007 sticks out to me but apparently someone did set a very quick record time in 2007.
...
records %>%
  ...
  labs(title = "Comparison of Record Times by Track Type",
       x = "Time",
       y = "Frequency")
...
ggplot(records, aes(time_per, fill = type)) +
  geom_histogram(alpha = 0.5, position = 'identity', bins = 30) +
  facet_wrap(~type, scales = 'free') +
  labs(title = "Comparison of Time per Lap by Track Type",
       x = "Time per Lap",
       y = "Frequency")
...
ggplot(records, aes(time_per, fill = type)) +
  geom_histogram(alpha = 0.5, position = 'identity', bins = 30) +
  facet_wrap(~type, scales = 'free') +
  labs(title = "Comparison of Time per Lap by Track Type (No Shortcuts)",
       x = "Time per Lap",
       y = "Frequency")
...
print(mean_time_by_system)
#I observe a difference in the mean time between the NTSC and the PAL. The NTSC is clearly smaller in mean time than the PAL. The reason for this is because the NTSC version runs at 60 FPS which is greater than 50 FPS. That is why the mean time for the NTSC is lower.
```
