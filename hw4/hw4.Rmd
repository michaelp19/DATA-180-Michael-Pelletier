...
```{r}
#office hours 3-4 on wednesday. 1 or 2333?
result <- records %>%
  arrange(record_duration) %>%
  mutate(date = c(0, diff(record_duration))) %>%
  summarise(count = n())
...
```{r}
#office hours ask for help
result <- records %>%
  arrange(record_duration) %>%
  mutate(date = c(0, diff(record_duration))) %>%
  filter(record_duration>3030) %>%
