...
#Label encoding(Assign a unique numerical value to each category in the "rep_education" column. This is useful when there is an ordinal relationship among categories).
```{r}
library(dplyr)
...
```
#One-Hot Encoding(Create binary columns for each category in "rep_education" where each column represents the presence or absence of a specific category).
```{r}
# Not done yet
library(dplyr)

your_dataset <- cleaned_dataset %>%
  mutate(across(rep_education, ~as.numeric(. == levels(factor(rep_education)))))

colnames(your_dataset)[grep("rep_education_", colnames(your_dataset))] <- levels(factor(your_dataset$rep_education))
